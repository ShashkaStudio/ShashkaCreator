...
**ShashkaCreator** абсолютно бесплатный для асболютно любых целей и абсолютно любого использования, да ещё и с открытым исходным кодом. Скрывать там буквально нечего: пару файлов, которые открываются даже через обычный блокнот. 
...
# Есть ли логотип?
Да, есть. Логотипом **ShashkaCreator** является **Jeff.bmp**(см. загруженные файлы).
# ShashkaCreator
## Окно
...
* **Примечание: по умолчанию библиотека ищет видеофайлы в папке с .exe файлом вашего проекта.**
...
Началось всё с того, что в июне 2025 года у меня появилось желание создать свой движок. До этого я делал разные механики в Unity, но мне всегда хотелось узнать, как всё работает изнутри. При чём я хотел попробовать сделать движок, имея только стандартные библиотеки под рукой. Но проблема была в том, что все гайды на ютубе начинались с "установки обязательной библиотеки, без которой ничего не работает", обычно это был GLFW, SFML или ещё что похуже. Естественно, это мне не подходило.  

Нормальных советов в интернете ну просто не было. На мою идею я получал ответ в духе: "да это невозможно, всё равно придётся что-то для графики скачивать". Но всё изменилось, когда я нашёл ютуб канал **BeginEnd**. Нет, чуваки, это не реклама, да и автор канала вряд-ли вообще знает о существовании меня и моей библиотеки. Собственно на канале автор выкладывал просто шедевральные и понятные уроки по C\C++ **Windows.h**, и буквально с помощью стандартной библиоеки он показал всё необходимое для создания 2д движка.   

Конечно, с нуля учиться работать в WinAPI было тяжело, но спустя время я всё-таки смог сделать первую функционирующую версию **ShashkaCreator**, и в ней буквально можно было только квадраты рисовать. Случилось это 29 январа 2026 года. Почему прошло столько времени? Ну потому что смотрел видео и писал код я не каждый день(а иногда и не каждую неделю), а за один день я тратил 2-3 часа на разработку. Но в итоге спустя время библиотека обрела нормальный вид и имеет всё необходимое для создания 2д графических приложений.  
