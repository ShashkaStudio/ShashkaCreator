...
**ShashkaCreator** абсолютно бесплатный для асболютно любых целей и абсолютно любого использования. Скрывать там буквально нечего: пару файлов, которые открываются даже через обычный блокнот. Буду особенно рад модам и переделкам. 
# Как установить библиотеку?
Добавьте файлы **ShashkaCreator.cpp** и **ShashkaCreator.h** в ваш C++ проект и подключите библиотеку в основном файле как **#include "ShashkaCreator.h"**. Таже для работы вам понадобится подключить к проекту ещё 4 файла: **libgdi32.a**, **libmsimg32.a**, **libwinmm.a** и **libws2_32.a**. Эти библиотеки есть по умолчанию в каждом **Windows**, так что ничего дополнительного скачивать не придётся. **Советую использовать MinGW в качестве компилятора.**
...
Началось всё с того, что в июне 2025 года у меня появилось желание создать свой движок. До этого я делал разные механики в Unity, но мне всегда хотелось узнать, как всё работает изнутри. При чём я хотел попробовать сделать движок, имея только стандартные библиотеки под рукой. Но проблема была в том, что все гайды на ютубе начинались с "установки обязательной библиотеки, без которой ничего не работает", обычно это был GLFW или SFML. Для меня такой подход был ужасным, потому что готовая библиотека неэффективно выполняла всю работу за меня.   
