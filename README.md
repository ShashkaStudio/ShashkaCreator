...
## Пролог
Чтобы установить этот модуль, переместите файл **ShashkaAlgorithm.h** в папку с вашим C++ проектом и подключите библиотеку в основном файле как **#include "ShashkaAlgorithm.h"**. Для работы с этим модулем подключать основной **ShashkaCreator.h** не нужно. Сам ShashkaAlgorithm имеет свой динамический массив и функции для работы с ним(что-то по типу **vector**). Так что если вы просто подключите **vector** и **algorithm**, то принципиальной разницы сильно не почувствуете.
## Буффер
