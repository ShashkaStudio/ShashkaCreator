...
Абсолютно любой человек может использовать ShashkaCreator в асболютно любых целях. Там буквально нечего скрывать: библиотека состоит из пары файлов, которые открываются даже через стандартный блокнот, а для понимания исходного кода достаточно уметь работать в библиотеке **Windows.h**, материалы которой есть в свободном доступе на сайте **MicrosoftLean**. Так что владейте, пользуйтесь, распоряжайтесь.
