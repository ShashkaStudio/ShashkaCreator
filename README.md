...
Добавьте файлы **ShashkaCreator.cpp** и **ShashkaCreator.h** в ваш C++ проект и подключите библиотеку в основном файле как **#include "ShashkaCreator.h"**. Таже для работы вам понадобится подключить к проекту ещё 4 файла: **libgdi32.a**, **libmsimg32.a** и **libwinmm.a**. Эти библиотеки есть по умолчанию в каждом **Windows**, так что ничего дополнительного скачивать не придётся.
# Какие технические возможности библиотеки?
Использует процессор для отрисовки графики(да-да, представляете, вам **не потребуется мощнейшая видеокарта**), весит около **40 килобайт**, крайне просто устанавливается, не зависит от других библиотек, быстро и оптимально работает на слабых устройствах, запускается на любом **Windows**, старше чем **Windows 2000**. Звучит идеально, не правда ли? Но есть и минусы. Библиотека заточена строго под **Windows**, так что она не кроссплатформенная. Также вы не сможете сделать условно свой клон **Hollow Knight** с тонной частиц и эффектов, потому что у **GDI** есть жёсткие ограничения по количеству отрисовываемых объектов, и лимит в **10000** вы просто не пробьёте. Так что подойдёт она для создания мелких и средних проектов, а также для тех, кто хочет научиться работать в **WinAPI** или просто побаловаться.
# Какая сейчас версия?
Версии именуются месяцем и годом создания. Начал я делать библиотеку 29 января 2026 года, то есть первая версия была **January2026**. Переодически я делаю обновления для библиотеки и выкладываю новые версии. Последняя версия - **September2026**.
# Есть ли логотип?
Да, есть. Логотипом **ShashkaCreator** является **Jeff.bmp**(см. загруженные файлы). Также у нас есть своя "национальная музыка" - **Kevin MacLeod - Aces High**.
# ShashkaCreator
## Окно
...
* **active** - активен ли примитив. При значении 0 примитив не будет отрисовываться в окне.
...
```bool GetKeyDown(int KeyCode);``` - возвращает 1, если заданная клавиша была **нажата один раз**.  
```bool GetKey(int KeyCode);``` - возвращает 1, если заданная клавиша **зажата в данный момент**.  
```bool GetKeyUp(int KeyCode);``` - возвращает 1, если заданная клавиша **была отпущена после зажатия**.  
...
```sound имя = {std::string path, std::string alias};```
* **path** - путь к звуковому файлу. **Не используйте пробелы и нелатинские буквы!**
* **alias** - название звука. Название может быть любым, но лучше используйте латиницу.
* **Примечание: по умолчанию библиотека ищет звуковые файлы в папке с .exe файлом вашего проекта(Code::Blocks, DevC++). Может отличаться в разных IDE!**
...
## Видео
Библиотека умеет работать с видеофайлами с рашсирением **.avi**. Давайте создадим видео.

```video имя = {std::string path, std::string alias, int x, int y, int width, int height, bool active};```
* **path** - путь к видео файлу. **Не используйте пробелы и нелатинские буквы!**
...
* **active** - активно ли видео. При значении 0 видео не будет проигрываться.
* **Примечание: по умолчанию библиотека ищет видеофайлы в папке с .exe файлом вашего проекта(Code::Blocks, DevC++). Может отличаться в разных IDE!**
...
```sprite имя = {const char* path, HBITMAP image, float x, float y, float width, float height, float angle, unsigned char alpha, COLORREF keyColor, bool active};```  
* **path** - путь к графическому изображению. **Не используйте пробелы и нелатинские буквы!**
...
* **active** - активен ли спрайт. При значении 0 спрайт не будет отрисовываться в окне.
* **Примечание: по умолчанию библиотека ищет графические изображения в папке с .exe файлом вашего проекта(Code::Blocks, DevC++). Может отличаться в разных IDE!**
...
```int GetMouseWheel();``` - возвращает UpMouse, если колёсико мыши прокручивается вверх, DownMouse, если колёсико мыши прокручивается вниз, и NormalMouse во всех остальных случаях.
...
* **active** - активен ли текст. При значении 0 текст не будет отрисовываться в окне.
* **Примечание: по умолчанию библиотека ищет шрифты в папке с .exe файлом вашего проекта(Code::Blocks, DevC++). Может отличаться в разных IDE!**
...
```std::string toText(int variable);``` - тоже самое, что и std::to_string(); Есть также перегрузка для bool, float, double, long long и short.
...
```bool IsCollide(square &objectA, square &objectB);``` - возвращает 1, если два примитива столкнулись.
...
* **active** - активен ли триггер. При значении 0 триггер не будет работать.
...
```UIButton имя = {square box, RGB normalColor, RGB hoverColor, text label, bool isHovered, bool isClicked, float clickTimer};```  
...
```inputfield имя = {square box, text label, std::string content, int maxChars, bool isFocused};```
...
```scrollbar имя = {square box, square slider, int direction, bool isDragged, float filled};```
...
```menu имя = {std::string content, HMENU hmn};```
* **content** - надпись на кнопке.
* **Последний параметр не заполняйте.**

```submenu имя = {std::string content, int id};```
* **content** - надпись на кнопке.
...
```bool ClickMenu(const submenu& object);``` - возвращает 1, если нажато заданное подменю.
...
```message имя = {std::string title, std::string content, UINT icon, UINT type, bool active};```
...
* **active** - активно ли диалоговое окно. При значении 0 оно не будет отрисовываться в окне.
...
```bool RenameFile(const std::string& sourcePath, const std::string& destPath);``` - переименовывает заданный файл и возвращает 1, если операция прошла успешно.

**Примечание: если в качестве путя к файлу указывать только название файла(без самого пути к нему, например: "Sponge.txt"), то библиотека будет искать его в папке с .exe файлом проекта(Code::Blocks, DevC++). Может отличаться в разных IDE! Если вы хотите указать полноценный путь к файлу, то делайте это через двойной слеш(например: "D:\\Documents\\"). Не используйте пробелы и нелатинские буквы!**
...
```
## Текстовые файлы
Небольшого отдельного внимания заслуживают текстовые файлы. Их отличие от обычных файлов заключается в том, что их можно изменять и читать. Быстренько пробежимся по всем функциями.

```bool CreateTextFile(const std::string& path);``` - создаёт текстовый файл по заданному пути и возвращает 1, если операция прошла успешно.   
```bool SaveTextToFile(const std::string& path, const std::string& content);``` - записывает заданный текст в заданный текстовый файл и возвращает 1, если операция прошла успешно.   
```std::string LoadTextFromFile(const std::string& path);``` - читает заданный текстовый файл и возвращает содержимое в виде строки.   
...
message msg = {"Загрузка текста", "", information, ok, true};
...
Start() {
    ...
    CreateTextFile("Sponge.txt");
    SaveTextToFile("Sponge.txt", "September2026");

    msg.content = LoadTextFromFile("Sponge.txt");

    DisplayWindow(Update);
}

void Update() {
    if (ShowMessage(mainWindow, msg) == idOk) {
        msg.active = false;
    }
...
```date имя = {int day, int month, int year};```
* **day** - день.
* **month** - месяц.
* **year** - год.
...
```
# Эпилог
Если вы дочитали до этого момента и не бросили, то поздравляю: это конец. Всё, теперь вы знакомы со всеми структурами и функциями в библиотеке и можете спокойно создавать графические приложения. Буду очень рад, если на основе ShashkaCreator будут создаваться моды или вообще другие библиотеки, а также прошу писать мне при обнаружении каких-либо багов и недоработок.

Что бы вам такого ещё рассказать... Может быть историю создания?

Началось всё с того, что в июне 2025 года у меня появилось желание создать свой движок. До этого я делал разные механики в Unity, но мне всегда хотелось узнать, как всё работает изнутри. При чём я хотел попробовать сделать движок, имея только стандартные библиотеки под рукой. Но проблема была в том, что все гайды на ютубе начинались с "установки обязательной библиотеки, без которой ничего не работает", обычно это был GLFW, SFML или ещё что похуже. Естественно, такой подход полнолстью ломал суть и интерес. Ведь что мы получаем в итоге? Правильно! Крайне неоптимизированный тяжёлый движок на кучу файлов, тонной зависимостей, сложной установкой и пожиранием видюхи просто ради того, чтобы фигуры рисовать.   

Нормальных советов в интернете ну просто не было. На мою идею я получал ответ в духе: "да это невозможно, всё равно придётся что-то для графики скачивать". Но всё изменилось, когда я нашёл ютуб канал **BeginEnd**. Нет, чуваки, это не реклама, да и автор канала вряд-ли вообще знает о существовании меня и моей библиотеки. Собственно на канале автор выкладывал просто шедевральные и понятные уроки по C\C++ и **Windows.h**, и буквально с помощью стандартной библиотеки он показал всё необходимое для создания полноценного 2д движка.   

Конечно, с нуля учиться работать в WinAPI было тяжело, но спустя время я всё-таки смог сделать первую функционирующую версию **ShashkaCreator**, и в ней буквально можно было только создавать окно и квадраты рисовать. Случилось это 29 январа 2026 года. Почему прошло столько времени? Ну потому что смотрел видео и писал код я не каждый день(а иногда и не каждую неделю), а за один день я тратил 2-3 часа на разработку. Но в итоге спустя время библиотека обрела нормальный вид и имеет всё необходимое для создания графических приложений.  

В итоге я получил то, чего хотел. Конец ли это? Для **ShashkaCreator** - да, а в общем - нет. Понятное дело, что эта версия имеет огромнейший минус - рендер на процессоре, из-за чего крупные игры с большим количеством объектов создавать на ней невозможно. И эта версия уже полностью укомплектована и добавлять в неё больше нечего (давайте называть её **Classic**). Но проблему эту можно исправить. Если не заброшу свой движок, то спустя время обязательно сделаю новую версию, в которой рендеринг уже окончательно будет завязан на видеокарте, но оптимизация также будет оставаться на высшем уровне. 

Ну а на этом всё. Ещё раз огромное спасибо каналу **BeginEnd**. 
