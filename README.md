...
## Shashka Creator — это библиотека с открытым исходным кодом на C++ для создания 2D графических приложений под Windows. Библиотека написана в Code::Blocks с использованием стандартных библиотек WinAPI и состоит из нескольких модулей.
...
Для отрисовки графики библиотека задействует процессор, так что вполне подойдёт для мелких и средних проектов. Что-то огромное по типу **Hollow Knight** вы конечно не сделаете, ибо **WinAPI** позволяет одновременно рисовать максимум **65535** объектов. Однако, у библиотеки есть очень много плюсов: вес всего около 40 килобайт, быстрота работы, крайне простая установка, оптимальная работа на слабых устройствах, независимость от сторонних библиотек. Всё, что вам нужно - это иметь устройство с **Windows** старше чем **Windows 2000** и хотя бы встроенную видеокарту.
...
```
## Делать документацию по физике и файловой системе мне лень, так что дальше разбирайтесь сами. Может быть когда-то вернусь сюда и допишу.
